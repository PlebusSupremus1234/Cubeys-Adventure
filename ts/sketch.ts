...
function draw() {
    ...
    if (keysDown[0]) player.move("left");
    if (keysDown[1]) player.move("right");
    ...
    if (showlvlselect) document.getElementById("modal").style.display = "block";
...
function keyDown(k) {
    ...
    else if (k.key.toLowerCase() === "arrowright") keysDown[1] = true;
}
